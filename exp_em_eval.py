...
def main():
    ...
    for i, item in enumerate(prefix_data):
        ...
        unlearn_hiddens = get_all_layers_hidden(
            unlearn, inputs["input_ids"], inputs["attention_mask"], layer_list, position=-1
        )
        ...
        for layer in layer_list:
            ...
            s1_gen = generate_with_patch(full, tokenizer, prompt, layer, retain_hiddens[layer], max_new_tokens=20)
            ...
            s2_gen = generate_with_patch(full, tokenizer, prompt, layer, unlearn_hiddens[layer], max_new_tokens=20)
