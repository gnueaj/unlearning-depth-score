...
def compute_filtering_thresholds(pn_results_path: str) -> Dict[str, float]:
    ...
    for metric in sample_metrics:
        # UDS is stored as raw UDS (higher=better unlearning), but thresholding
        # in §4.2.1 is done on "knowledge score" where higher=more knowledge.
        # So for UDS, use 1-UDS while fitting the threshold.
        if metric == "uds":
            p_scores = [1 - results[m]['metrics'].get(metric) for m in p_models
                        if results[m].get('metrics', {}).get(metric) is not None]
            n_scores = [1 - results[m]['metrics'].get(metric) for m in n_models
                        if results[m].get('metrics', {}).get(metric) is not None]
        else:
...
def _is_low_lr_model(name: str, model_id: str) -> bool:
    """
    Detect low learning-rate checkpoints used in paper quantization setup.

    Supports common naming variants:
      - lr1e5  (repo short name convention for 1e-5)
      - lr1e-05 / lr1e-5
      - lr=1e-5
    """
    ...
    def __init__(
        ...
        for item in ds:
            ...
            if use_chat_template and hasattr(tokenizer, "apply_chat_template"):
                # Match Open-Unlearning chat formatting for Instruct models.
                messages = []
                if system_prompt:
                ...
                chat_ids = prompt_ids + answer_ids
...
def _metric_to_knowledge(metric: str, value: Optional[float]) -> Optional[float]:
    if value is None:
        return None
    # Robustness meta-eval uses raw metric values for all Open-Unlearning metrics.
    # Only UDS is inverted to align direction with "knowledge signal" (higher means
    # more residual knowledge), i.e., use (1 - UDS).
    if metric == "uds":
        return 1 - value
    return value
...
def main():
    ...
    parser.add_argument("--relearn_batch_size", type=int, default=4,
                        help="Per-device batch size for relearning finetune")
    parser.add_argument("--relearn_grad_accum", type=int, default=4,
                        help="Gradient accumulation for relearning finetune")
    parser.add_argument("--relearn_optim", type=str, default="adamw_torch",
                        help="Optimizer for relearning finetune (e.g., adamw_torch, paged_adamw_32bit)")
    ...
    parser.add_argument("--attn_implementation", type=str, default=None,
                        help="Attention impl for model loading: eager, sdpa, flash_attention_2")
    parser.add_argument("--quant_low_lr_only", action="store_true",
                        help="Run quantization only on low-lr checkpoints (paper appendix setup)")
    parser.add_argument("--paper_protocol", action="store_true",
                        help="Enable paper-like defaults (flash_attention_2, paged_adamw_32bit, "
                             "relearn batch=32, quant_low_lr_only)")
    args = parser.parse_args()

    if args.paper_protocol:
        # Apply paper-like defaults only when user didn't override.
        if args.attn_implementation is None:
            args.attn_implementation = "flash_attention_2"
        if args.relearn_optim == "adamw_torch":
            args.relearn_optim = "paged_adamw_32bit"
        if args.relearn_batch_size == 4 and args.relearn_grad_accum == 4:
            args.relearn_batch_size = 32
            args.relearn_grad_accum = 1
        args.quant_low_lr_only = True

    # FlashAttention2 may be unavailable in the runtime. Fall back to sdpa
    # so paper_protocol can still run end-to-end without manual intervention.
    if args.attn_implementation == "flash_attention_2":
        if importlib.util.find_spec("flash_attn") is None:
            print("[WARN] flash_attn not installed; falling back attn_implementation=sdpa")
    ...
    print(
        ...
        + (
            "SKIP"
            if args.skip_relearning
            else (
                f"lr={args.relearn_lr}, epochs={args.relearn_epochs}, "
                f"bs={args.relearn_batch_size}, ga={args.relearn_grad_accum}, optim={args.relearn_optim}"
            )
        )
    )
    quant_str = "SKIP" if args.skip_quantization else "4-bit NF4"
    if not args.skip_quantization and args.quant_low_lr_only:
        quant_str += " (low-lr-only)"
    print(f"Quantization: {quant_str}")
    print(f"Attn implementation: {args.attn_implementation or 'default'}")
        ...
        if need_retain_model:
            ...
            if metrics_missing_ret_after:
                ...
                finetune_on_forget10(
                    ...
                    grad_accum=args.relearn_grad_accum,
                    optim=args.relearn_optim,
                    use_chat_template=args.use_chat_template,
                    system_prompt=args.system_prompt or None,
                    date_string=args.date_string or None,
                )
    ...
    if args.model_start is not None or args.model_end is not None:
        s = args.model_start if args.model_start is not None else 0
        e = args.model_end if args.model_end is not None else len(eval_items)
    ...
    for mi, (name, model_id) in enumerate(eval_items):
        ...
        # Invalidate cached relearning results if protocol changed.
        relearn_cfg_expected = {
            "lr": args.relearn_lr,
        ...
        relearn_cfg_cached = model_results.get("relearning_details", {})
        relearn_cache_stale = any(
            relearn_cfg_cached.get(k) != v for k, v in relearn_cfg_expected.items()
        )
        if relearn_cache_stale:
            cached_R = {}
            model_results["relearning_R"] = {}
            model_results.pop("metrics_after_relearn", None)
            metrics_missing_R = metrics.copy()
            print("\n[2/3] Relearning cache invalidated (protocol changed), recomputing...")
        ...
        if not args.skip_relearning and metrics_missing_R:
            ...
            model_results["relearning_details"] = relearn_cfg_expected
        ...
        quant_scope_expected = "low_lr_only" if args.quant_low_lr_only else "all_models"
        quant_details_cached = model_results.get("quantization_details", {})
        quant_cache_stale = (
            quant_details_cached.get("quant_type") not in (None, "nf4_4bit")
            or quant_details_cached.get("scope") not in (None, quant_scope_expected)
        )
        # If eligibility changed (e.g., low-lr detection bug fix), force recompute.
        cached_skipped = quant_details_cached.get("skipped")
        now_eligible = (not args.quant_low_lr_only) or _is_low_lr_model(name, model_id)
        if cached_skipped is not None and cached_skipped == now_eligible:
            quant_cache_stale = True
        if quant_cache_stale:
            cached_Q = {}
            model_results["quantization_Q"] = {}
            metrics_missing_Q = metrics.copy()
            print("\n[3/3] Quantization cache invalidated (protocol/scope changed), recomputing...")

        quantization_eligible = True
        if args.quant_low_lr_only and not _is_low_lr_model(name, model_id):
            quantization_eligible = False
            # Force all Q metrics to None for out-of-scope checkpoints.
            cached_Q = {metric: None for metric in metrics}
            model_results["quantization_Q"] = cached_Q
            model_results["quantization_details"] = {
                "quant_type": "nf4_4bit",
                "scope": "low_lr_only",
                "skipped": True,
                "reason": "non_low_lr_checkpoint",
            }

        if not args.skip_quantization and metrics_missing_Q:
            if not quantization_eligible:
                print("\n[3/3] Quantization: SKIPPED (non-low-lr checkpoint under --quant_low_lr_only)")
                source_model = None
            ...
            model_results["quantization_details"] = {
                "quant_type": "nf4_4bit",
                "scope": "low_lr_only" if args.quant_low_lr_only else "all_models",
            }
    ...
    for name in eval_models:
        ...
        for m in metrics:
            # Relearning (R): utility + faithfulness threshold filters
            pass_relearning = True
            if args.filter_utility and name in utility_filtered:
                pass_relearning = False
            if pass_relearning and args.filter_insufficient and filter_thresholds:
            ...
            # Quantization (Q): lr=1e-5-only policy is enforced at evaluation stage.
            # No additional utility/threshold filtering is applied in aggregation.
